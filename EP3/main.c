    ...
    */
    ...
    while(fgets(linha, TAM_MAX_LINHA, fTexto) != NULL ) {
        ...
        while( word != NULL ) {
             ...
             */
            if ( buscaNo(raiz, cpypalavra) || buscaNo(raiz,cpypalavra2))
                printf("%s ", word);
            else {
                 ...
                 * 
                 */
                // VERIFICA SE A PALAVRA JÁ ESTÁ NA LISTA DE PALAVRAS INCORRETAS
                if(!buscaPalavraFila(incorretas, word)) 
                    enqueue(incorretas, word);
                aux1++;
            }
            aux2++;
     ...
     * 
     */
    while(!filaVazia(incorretas)){
        imprimePalavraErrada((palavraFront(incorretas)));
        printf(": ");
        imprimeSugestao(raiz, palavraFront(incorretas));
        printf("\n");
        dequeue(incorretas);
    }
