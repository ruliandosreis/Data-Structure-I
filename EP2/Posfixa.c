...
Fila *infixaParaPosfixa(Fila *infixa) {
    ...
    while(front(infixa)){
        Objeto *atual = front(infixa);
        if(atual->categoria == INT || atual->categoria == FLOAT){
            // CASO INTEIRO OU FLOAT
            enqueue(filaSaida, atual);
        }else if(atual->categoria == ABRE_PARENTESES){
            // CASO ABRE PARENTESES
            empilha(pilhaOperadores, atual);
        }else if(atual->categoria == FECHA_PARENTESES){
            // CASO FECHA PARENTESES
            Objeto *percorrePilha = topoPilha(pilhaOperadores);
            while(percorrePilha->categoria != ABRE_PARENTESES){
                enqueue(filaSaida,percorrePilha);
                desempilha(pilhaOperadores);

                percorrePilha = topoPilha(pilhaOperadores);
            }
            desempilha(pilhaOperadores); // RETIRA O ABRE PARENTESES
        }else{
            // CASO EXPONENCIAÇÃO E MENOS UNÁRIO (PRIORIDADE MÁXIMA)
            if(atual->categoria == 0 || atual->categoria == 6){
                empilha(pilhaOperadores, atual);
            }
            else{
                while(true){
                    // RESTANTE DOS CASOS DE OPERADORES
                    Objeto *percorreOperadores = topoPilha(pilhaOperadores);
                    if(percorreOperadores == NULL || getPrecedenciaOperadores(percorreOperadores->categoria) < getPrecedenciaOperadores(atual->categoria)){
                        break;
                    }
                    // CASO A PRIORIDADE SEJA MAIOR
                    enqueue(filaSaida, percorreOperadores);
                    desempilha(pilhaOperadores);
                }
                // CASO A PRIORIDADE SEJA MENOR
                empilha(pilhaOperadores, atual);
            }
        }
        dequeue(infixa);
    }
    // CASO AINDA HAJA OPERADORES NA PILHA
