...
Imagem *alocaImagem(int largura, int altura){
    Imagem *imagem = mallocSafe(sizeof(Imagem));

    imagem->altura = altura;
    imagem->largura = largura;

    imagem->pixel = mallocSafe(altura*sizeof(Pixel*));
    for(int i=0; i< altura; i++){
        imagem->pixel[i] = mallocSafe(largura*sizeof(Pixel));
    }
    return imagem;
}
...
Imagem* copiaImagem(Imagem *origem){
    if(origem){
        Imagem *copiaImagem = mallocSafe(sizeof(Imagem));
        copiaImagem->altura = origem->altura;
        copiaImagem->largura = origem->largura;

        copiaImagem->pixel = mallocSafe(origem->altura*sizeof(Pixel*));
        for(int i=0; i< origem->altura; i++){
            copiaImagem->pixel[i] = mallocSafe(origem->largura*sizeof(Pixel));
        }
        for(int q=0; q< origem->altura; q++){
            for(int r=0; r< origem->largura; r++){
                copiaImagem->pixel[q][r] = origem->pixel[q][r];
            }
        }
        return copiaImagem;
    }
...
void salvaImagem(Imagem *img, const char *nomeArquivo){
    ...
    for(int i=0; i < img->altura; i++){
        for(int j=0; j < img->largura; j++){
            fprintf(arquivo,"%d\n",obtemPixel(img,i,j).cor[RED]); // R
            fprintf(arquivo,"%d\n",obtemPixel(img,i,j).cor[GREEN]); // G
            fprintf(arquivo,"%d\n",obtemPixel(img,i,j).cor[BLUE]); // B
        }
    }
    fclose(arquivo);
